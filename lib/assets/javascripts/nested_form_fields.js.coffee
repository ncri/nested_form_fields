...
nested_form_fields.bind_nested_forms_links = () ->
  ...
  $('body').on 'click', '.add_nested_fields_link', (event, additional_data) ->
    ...
    max_index = Math.max.apply(Math, $(this).parents('form').eq(0).find("[id^=\"#{association_path}_attributes\"]").map(-> parseInt(this.id.replace(association_path + '_attributes_', '').split('/')[0])))
